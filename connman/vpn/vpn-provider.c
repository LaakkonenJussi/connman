...
static gboolean do_connect_timeout_function(gpointer data) {
	...
	if (!t_data->conn || !t_data->msg || !t_data->provider) {
		DBG("insufficient data conn %p msg %p data %p removing from main loop",
								...
								void *data)
{
	...
	if (!connman_online) {
		...
		if (state_query_done) {
			...
			DBG("Provider %s start delayed because connman state not queried",
				provider->identifier);
			...
			if (!do_connect_timeout) {
				...
				DBG("Timeout function already added for do_connect");
			}
...
static gboolean run_get_connman_state()
{
	const char *path = "/";
...
static void get_connman_state()
{
	...
	get_connman_state_timeout = 
		g_timeout_add(500, run_get_connman_state, NULL);
}
