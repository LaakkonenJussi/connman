...
static bool allow_service_over_default(struct connman_service *new_service)
{
	...
	if (is_online(current_default) && !is_online(new_service)) {
...
static void default_changed(void)
{
	...
	if (service == current_default) {
		...
		/*
		 * Even if the default was not changed a VPN, which is not being
		 * set as default route, may have been added triggering this so
		 * notifier should be informed to take account the DNS servers.
		 * Or this is caused by a technology change.
		 */
		goto notify;
	}
	...
	if (!service) {
		...
		if (service == current_default) {
			...
			/*
			 * Force to update even though the default will not be
			 * changed. May be caused by a non-default route VPN
			 * being added, or a technology change not resulting
			 * re-ordering of services because of preference.
			 * Important for including the DNS servers of a such VPN
			 * as it depends on the default
			 */
			goto notify;
		}
	}
...
int __connman_service_get_depending_vpn_index(struct connman_service *service)
{
	GList *iter = NULL;
	...
	for (iter = service_list ; iter ; iter = iter->next) {
		...
		/*
		 * Return the index of the VPN if it uses service as transport
		 * service and is not being used as default route. This way
		 * the dnsproxy can forward messages to the DNS servers
		 * reported by a VPN not acting as default route for all
		 * traffic.
		 */
		if (vpn->type == CONNMAN_SERVICE_TYPE_VPN &&
			vpn->depends_on == service && is_connected(vpn) &&
			!__connman_service_is_default_route(vpn))
			return connman_provider_get_index(vpn->provider);
	}
				...
				bool preferred)
{
	...
	for (list = services; list; list = list->next) {
		 ...
		 */
		if (!active_count) {
			if (!preferred || (preferred && preferred_found)) {
...
static gboolean run_vpn_auto_connect(gpointer data) {
	 ...
	 * stop auto connection or remove the vpn_autoconnect_timeout to keep
	 ...
	 */
	if (!service || !is_online(service) ||
		(service->type == CONNMAN_SERVICE_TYPE_VPN &&
		is_connected(service))) {

		DBG("Stopping, default service %s connected %d",
			service ? service->identifier : "NULL",
			service ? is_connected(current_default) : -1);
		return FALSE;
	}
