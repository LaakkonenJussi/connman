...
#define VPN_AUTOCONNECT_TIMEOUT_IDLE 2
...
static gboolean run_vpn_auto_connect(gpointer data) {
	...
	/*
	 * If there is no VPN to automatically connect increase delay to 2s.
	 * Otherwise if there are autoconnectable vpns re-add with default delay
	 */
	if (!autoconnectable_vpns && delay != VPN_AUTOCONNECT_TIMEOUT_IDLE) {
		delay = VPN_AUTOCONNECT_TIMEOUT_IDLE;
		DBG("no autoconnectable VPNs found, increasing delay to %ds",
			delay);
		goto retry;
	} else if (autoconnectable_vpns &&
		delay > VPN_AUTOCONNECT_TIMEOUT_DEFAULT) {
		delay = VPN_AUTOCONNECT_TIMEOUT_DEFAULT;
		DBG("reset delay back to default %ds", delay);
		goto retry;
	}
	...
	/*
	 * Re add this to main loop. TODO Implement a way to increase the
	 * delay in case there are errors in attempting VPN connection after
	 * connmand has been modified to get errors from VPNs. Delay is added
	 * as user data indicating the delay of the previous add to main loop
	 * and to tell that this is not the first run started via
	 * vpn_auto_connect().
	 */
retry:
