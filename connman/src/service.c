	...
	struct connman_service *b)
{
	...
	/*
	 * If either or both are NULL or the types match the service b can be
	 * selected over a because ...
	 */
...
static void default_changed(void)
{
	...
	if (!service) {
		...
		__connman_resolver_redo_servers(
			__connman_service_get_index(current_default));
	}
	...
	if (service && !allow_service_over_default(service)) {
		...
		if (current_default &&
			 ...
			 */
			switch_default_service(service,
				current_default->depends_on);
			switch_default_service(current_default->depends_on,
				current_default);
		} else {
			switch_default_service(service, current_default);
		}
	...
	if (current_default) {
		if (current_default->type != CONNMAN_SERVICE_TYPE_VPN) {
			...
			if (!current_default->depends_on) {
				DBG("disconnect VPN with no dependency");
				disconnect_vpn_service(current_default, NULL);
			}
			/*
			 * When current_default is VPN and it is not depending
			 * on the new default service the VPN must be
			 * disconnected. Otherwise the change is not done as
			 * VPN depends on the service that would be the default
			 */
			if (service &&
				!is_vpn_dependent(service, current_default)) {
				DBG("disconnect VPN as current_default");
				disconnect_vpn_service(current_default, NULL);
			} else {
