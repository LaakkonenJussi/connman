...
void disconnect_vpn_service(struct connman_service *service, void* user_data)
{
	if (service->type == CONNMAN_SERVICE_TYPE_VPN) {
		...
		case CONNMAN_SERVICE_STATE_READY:
			if (service->provider) {
				...
				connman_error("disconnecting VPN %s state %d", 
...
static void default_changed(void)
{
	...
	if (service) {
		...
		if (service->type != CONNMAN_SERVICE_TYPE_VPN) {
			/* Disconnect VPNs to force VPNs use the default service */
