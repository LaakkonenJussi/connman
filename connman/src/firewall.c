...
static int call_connman_iptables_append(int type, const char *table_name,
	const char *chain_name, const char *rule_spec)
{
	switch (type) {
	case AF_INET:
		return __connman_iptables_append(table_name, chain_name,
						rule_spec);
	case AF_INET6:
		return __connman_ip6tables_append(table_name, chain_name,
						rule_spec);
	default:
		return -EINVAL;
	}
...
static int call_connman_iptables_insert(int type, const char *table_name,
	const char *chain_name, const char *rule)
{
	switch (type) {
	case AF_INET:
		return __connman_iptables_insert(table_name, chain_name, rule);
	case AF_INET6:
		return __connman_ip6tables_insert(table_name, chain_name, rule);
	default:
		return -EINVAL;
	}
}

static int call_connman_iptables_delete(int type, const char *table_name,
	const char *chain_name, const char *rule_spec)
{
	switch (type) {
	case AF_INET:
		return __connman_iptables_delete(table_name, chain_name,
						rule_spec);
	case AF_INET6:
		return __connman_ip6tables_delete(table_name, chain_name,
						rule_spec);
	default:
		return -EINVAL;
	}
...
static int call_connman_iptables_new_chain(int type, const char *table_name,
	const char *chain_name)
{
	switch (type) {
	case AF_INET:
		return __connman_iptables_new_chain(table_name, chain_name);
	case AF_INET6:
		return __connman_ip6tables_new_chain(table_name, chain_name);
	default:
		return -EINVAL;
	}
}

static int call_connman_iptables_delete_chain(int type, const char *table_name,
	const char *chain_name)
{
	switch (type) {
	case AF_INET:
		return __connman_iptables_delete_chain(table_name, chain_name);
	case AF_INET6:
		return __connman_ip6tables_delete_chain(table_name, chain_name);
	default:
		return -EINVAL;
	}
}

static int call_connman_iptables_commit(int type, const char *table_name)
{
	switch (type) {
	case AF_INET:
		return __connman_iptables_commit(table_name);
	case AF_INET6:
		return __connman_ip6tables_commit(table_name);
	default:
		return -EINVAL;
	}
}

static int call_connman_iptables_flush_chain(int type, const char *table_name,
	const char *chain_name)
{
	switch (type) {
	case AF_INET:
		return __connman_iptables_flush_chain(table_name, chain_name);
	case AF_INET6:
		return __connman_ip6tables_flush_chain(table_name, chain_name);
	default:
		return -EINVAL;
	}
}

static int call_connman_iptables_iterate_chains(int type,
	const char *table_name, connman_iptables_iterate_chains_cb_t cb,
	void *user_data)
{
	switch (type) {
	case AF_INET:
		return __connman_iptables_iterate_chains(table_name, cb,
						user_data);

	case AF_INET6:
		return __connman_ip6tables_iterate_chains(table_name, cb,
						user_data);
	default:
		return -EINVAL;
	}
...
static int insert_managed_chain(int type, const char *table_name, int id)
{
	...
	err = call_connman_iptables_new_chain(type, table_name, managed_chain);
	...
	err = call_connman_iptables_insert(type, table_name,
					builtin_chains[id], rule);

	g_free(rule);
	if (err < 0) {
		call_connman_iptables_delete_chain(type, table_name,
						managed_chain);
		goto out;
	}
...
static int delete_managed_chain(int type, const char *table_name, int id)
{
	...
	rule = g_strdup_printf("-j %s", managed_chain);
	err = call_connman_iptables_delete(type, table_name,
					builtin_chains[id], rule);
	g_free(rule);
	...
	err =  call_connman_iptables_delete_chain(type, table_name,
					managed_chain);
...
out:
	err = call_connman_iptables_append(type, table_name, chain, rule_spec);
...
static int delete_managed_rule(int type, const char *table_name,
				...
				const char *rule_spec)
 {
	...
	if (id < 0) {
		/* This chain is not managed */
		return call_connman_iptables_delete(type, table_name,
						chain_name, rule_spec);
	}

	managed_chain = g_strdup_printf("%s%s", CHAIN_PREFIX, chain_name);

	err = call_connman_iptables_delete(type, table_name, managed_chain,
				rule_spec);
...
static int firewall_enable_rule(struct fw_rule *rule)
{
	...
	err = call_connman_iptables_commit(rule->type, rule->table);
...
static void flush_table(int type, const char *table_name)
{
	...
	err = call_connman_iptables_iterate_chains(type, table_name,
	...
	for (list = chains; list; list = list->next) {
		...
		err = call_connman_iptables_flush_chain(type, table_name,
						managed_chain);
	...
	err = call_connman_iptables_commit(type, table_name);
	if (err < 0) {
