...
static int firewall_rule_sort(gconstpointer a, gconstpointer b)
{
	...
	/* Rules without config file are on top, system added rules */
	if (rule_a->config_file && !rule_b->config_file)
		return 1;
				...
				const char *rule_fmt, ...)
{
	...
	ctx->rules = g_list_insert_sorted(ctx->rules, rule, firewall_rule_sort);
	return rule->id;
}
...
static int init_all_dynamic_firewall_rules(void)
{
	...
	if (g_file_test(FIREWALLCONFIGDIR, G_FILE_TEST_IS_DIR)) {
		...
		while ((filename = g_dir_read_name(dir))) {
			...
			/*
			 * Add read files into sorted list of configuration
...
static int firewall_reload_configurations()
{
	...
	while ((filename = g_dir_read_name(dir))) {
		 ...
		 */
		read_files = g_list_insert_sorted(read_files,
					g_strdup(filename),
					(GCompareFunc)g_strcmp0);
	}
	...
	for (list_iter = g_list_first(read_files); list_iter;
				list_iter = list_iter->next) {
		filename = list_iter->data;
	...
	while (list_iter)
	{
		 ...
		 */
		if (!read_files || !g_list_find_custom(read_files, config_file,
					(GCompareFunc)g_strcmp0)) {
