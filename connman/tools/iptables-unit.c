...
static void test_managed_rule_exists(const char *chain, const char *rule)
{
	char *test_rule;

	if (rule[0] == '%') {
		test_rule = g_strconcat("-A connman-", chain, " ", &(rule[1]),
					NULL);
		assert_rule_not_exists("filter", test_rule);
		goto out;
	} else if (g_str_has_prefix(rule, "-p all")) {
		/* -p all is omitted in iptables-save output, skip it */
		test_rule = g_strconcat("-A connman-", chain, " ", &(rule[7]),
					NULL);
	} else {
		test_rule = g_strconcat("-A connman-", chain, " ", rule, NULL);
	}

	assert_rule_exists("filter", test_rule);

out:
	g_free(test_rule);
}

static void test_managed_rule_not_exists(const char *chain, const char *rule)
{
	char *test_rule;
...
static void test_firewall_managed_rules0(void)
{
	...
	for (i = 0; general_input[i]; i++)
		test_managed_rule_exists("INPUT", general_input[i]);

	for (i = 0; general_output[i]; i++)
		test_managed_rule_exists("OUTPUT", general_output[i]);

	for (i = 0; general_forward[i]; i++)
		test_managed_rule_exists("FORWARD", general_forward[i]);
	...
	for (i = 0; general_input[i]; i++)
		test_managed_rule_not_exists("INPUT", general_input[i]);

	for (i = 0; general_output[i]; i++)
		test_managed_rule_not_exists("OUTPUT", general_output[i]);

	for (i = 0; general_forward[i]; i++) 
		test_managed_rule_not_exists("FORWARD", general_forward[i]);

	assert_rule_not_exists("filter", "-A INPUT -j connman-INPUT");
	assert_rule_not_exists("filter", "-A OUTPUT -j connman-OUTPUT");
	assert_rule_not_exists("filter", "-A FORWARD -j connman-FORWARD");

	assert_rule_not_exists("filter", ":connman-INPUT - [0:0]");
	assert_rule_not_exists("filter", ":connman-OUTPUT - [0:0]");
	assert_rule_not_exists("filter", ":connman-FORWARD - [0:0]");
}
...
static void test_firewall_managed_rules1(void)
{
	...
	for (i = 0; eth_input[i]; i++)
		test_managed_rule_exists("INPUT", eth_input[i]);

	for (i = 0; eth_output[i]; i++)
		test_managed_rule_exists("OUTPUT", eth_output[i]);
	...
	for (i = 0; eth_input[i]; i++)
		test_managed_rule_not_exists("INPUT", eth_input[i]);

	for (i = 0; eth_output[i]; i++)
		test_managed_rule_not_exists("OUTPUT", eth_output[i]);
...
static void test_firewall_managed_rules2(void)
{
	...
	for (i = 0; cellular_input[i]; i++)
		test_managed_rule_exists("INPUT", cellular_input[i]);

	for (i = 0; cellular_output[i]; i++)
		test_managed_rule_exists("OUTPUT", cellular_output[i]);
	...
	for (i = 0; cellular_input[i]; i++)
		test_managed_rule_not_exists("INPUT", cellular_input[i]);

	for (i = 0; cellular_output[i]; i++)
		test_managed_rule_not_exists("OUTPUT", cellular_output[i]);
...
static void test_firewall_managed_invalid_rules1(void)
{
	...
	for (i = 0; invalid_eth_input[i]; i++)
		test_managed_rule_not_exists("INPUT", invalid_eth_input[i]);

	for (i = 0; invalid_eth_output[i]; i++)
		test_managed_rule_not_exists("OUTPUT", invalid_eth_output[i]);
...
int main(int argc, char *argv[])
{
	...
	g_test_add_func("/firewall/managed_prep", test_firewall_managed_prep);
	g_test_add_func("/firewall/managed0", test_firewall_managed_rules0);
	g_test_add_func("/firewall/managed1", test_firewall_managed_rules1);
	g_test_add_func("/firewall/managed2", test_firewall_managed_rules2);
	g_test_add_func("/firewall/managed_clean", test_firewall_managed_clean);
	g_test_add_func("/firewall/managed_invalid_prep",
				test_firewall_managed_invalid_prep);
	g_test_add_func("/firewall/managed_invalid0",
				test_firewall_managed_invalid_rules0);
	g_test_add_func("/firewall/managed_invalid1",
				test_firewall_managed_invalid_rules1);
	g_test_add_func("/firewall/managed_invalid_clean",
