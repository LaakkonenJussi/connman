...
static void test_ip6tables_chain2(void)
{
	...
	err = __connman_iptables_change_policy(AF_INET6, "filter", "INPUT", "DROP");
	g_assert(err == 0);

	err = __connman_iptables_commit(AF_INET6, "filter");
	g_assert(err == 0);

	err = __connman_iptables_change_policy(AF_INET6, "filter", "INPUT", "ACCEPT");
	g_assert(err == 0);
...
static void test_ip6tables_chain3(void)
{
	...
	err = __connman_iptables_delete_chain(AF_INET6, "filter", "user-chain-1");
	g_assert(err == 0);
	...
	err = __connman_iptables_delete_chain(AF_INET6, "filter", "user-chain-0");
	g_assert(err == 0);
