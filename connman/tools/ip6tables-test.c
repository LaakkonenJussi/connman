...
{
	enum ip6tables_command cmd = IPTABLES_COMMAND_UNKNOWN;
...
out:
	...
	case IPTABLES_COMMAND_APPEND:
		err = __connman_ip6tables_append(table, chain, rule);
		break;
	case IPTABLES_COMMAND_INSERT:
		err = __connman_ip6tables_insert(table, chain, rule);
		break;
	case IPTABLES_COMMAND_DELETE:
		err = __connman_ip6tables_delete(table, chain, rule);
		break;
	case IPTABLES_COMMAND_POLICY:
		err = __connman_ip6tables_change_policy(table, chain, rule);
		break;
	case IPTABLES_COMMAND_CHAIN_INSERT:
		err = __connman_ip6tables_new_chain(table, chain);
		break;
	case IPTABLES_COMMAND_CHAIN_DELETE:
		err = __connman_ip6tables_delete_chain(table, chain);
		break;
	case IPTABLES_COMMAND_CHAIN_FLUSH:
		err = __connman_ip6tables_flush_chain(table, chain);
		break;
	case IPTABLES_COMMAND_DUMP:
		__connman_log_init(argv[0], "*", false, false,
			"ip6tables-test", "1");
		err = __connman_ip6tables_dump(table);
		break;
	...
	err = __connman_ip6tables_commit(table);
	if (err < 0) {
